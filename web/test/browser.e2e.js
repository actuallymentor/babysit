...
try {
    ...
    await page.type( `input[name="access-key"]`, `incorrect-secret` )
    await page.click( `button[type="submit"]` )
    await page.waitForSelector( `[role="alert"]` )
    assert.equal( await page.$( `a[href="/sessions/mobile-session"]` ), null )
    await page.click( `input[name="access-key"]` )
    ...
    const initial_state = readFileSync( state_file, `utf8` )
    unlinkSync( state_file )
    await page.waitForFunction( () => document.querySelector( `main` ).textContent.includes( `No live sessions.` ) )
    assert.match( await page.$eval( `main`, element => element.textContent ), /0 live sessions/ )
    writeFileSync( state_file, initial_state )
    await page.waitForSelector( `a[href="/sessions/mobile-session"]` )
    assert.match( await page.$eval( `main`, element => element.textContent ), /1 live session/ )

    // The hamburger opens a full-height side drawer, independent of header/content geometry.
    const open_menu = async () => {
        await page.click( `[aria-label="App menu"]` )
        await page.waitForFunction( () => {
            const drawer = document.querySelector( `#app-menu` )
            return drawer.open && drawer.getAnimations().every( animation => animation.playState === `finished` )
        } )
    }
    const assert_drawer_fits = async () => {
        const bounds = await page.$eval( `#app-menu`, element => {
            const { top, bottom, right, width } = element.getBoundingClientRect()
            return { top, bottom, right, width, viewport_height: innerHeight, viewport_width: innerWidth }
        } )
        assert.equal( bounds.top, 0 )
        assert.equal( bounds.bottom, bounds.viewport_height )
        assert.equal( bounds.right, bounds.viewport_width )
        assert.ok( bounds.width <= bounds.viewport_width )
    }
    await open_menu()
    await assert_drawer_fits()
    assert.equal( await page.$eval( `#app-menu`, element => element.contains( document.activeElement ) ), true )
    assert.equal( await page.$eval( `body`, element => element.style.overflow ), `hidden` )
    await page.keyboard.press( `Tab` )
    assert.equal( await page.$eval( `[aria-label="Theme"]`, element => element === document.activeElement ), true )
    await page.keyboard.down( `Shift` )
    await page.keyboard.press( `Tab` )
    await page.keyboard.up( `Shift` )
    assert.equal( await page.$eval( `[aria-label="Close menu"]`, element => element === document.activeElement ), true )
    // Native dialog may cycle through browser chrome, but never the underlying app.
    for( let step = 0; step < 12; step++ ) {
        await page.keyboard.press( `Tab` )
        assert.equal( await page.$eval( `#app-menu`, element => document.activeElement === document.body || element.contains( document.activeElement ) ), true )
    }
    await page.keyboard.press( `Escape` )
    await page.waitForFunction( () => !document.querySelector( `#app-menu` ).open )
    assert.equal( await page.$eval( `[aria-label="App menu"]`, element => element === document.activeElement ), true )
    assert.equal( await page.$eval( `body`, element => element.style.overflow ), `` )
    ...
    await open_menu()
    await Promise.all( [ page.waitForNavigation( { waitUntil: `networkidle0` } ), page.click( `button[title="Clear the app cache and reload"]` ) ] )
    await page.waitForSelector( `textarea` )
    await open_menu()
    await page.locator( `::-p-text(Log out)` ).click()
    await page.waitForSelector( `input[name="access-key"]` )
    assert.equal( await page.$( `textarea` ), null )
    assert.equal( await page.$eval( `body`, element => element.style.overflow ), `` )
    await page.reload( { waitUntil: `networkidle0` } )
    await page.waitForSelector( `input[name="access-key"]` )
