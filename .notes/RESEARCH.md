...
- **Model**: `--model provider/model`; credentials are scoped to the provider prefix. Babysit resolves explicit CLI model → configured route model → enabled, non-disabled stored provider → provider-compatible frontier fallback. With multiple recognized credentials, the newest stored provider wins unless routing config orders them. Verified OpenCode 1.18.19 and a real OpenRouter request on 2026-08-22: direct OpenAI uses `openai/gpt-5.6-sol`, while OpenRouter must use `openrouter/openai/gpt-5.6-sol`. No stored provider retains the direct-OpenAI default for `.babysitrc` API-key users.
