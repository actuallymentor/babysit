...
- Chrome's Linux sandbox works as the non-root image user when Docker permits its namespace calls. The tested runtime materializes a current Moby-default-derived allowlist with masked `clone` and `unshare` rules for user/PID/network namespaces, plus a 1 GiB `/dev/shm`; it retains seccomp, grants no `SYS_ADMIN`, and does not disable Chrome's own sandbox. Each production launch uses a random private client-local profile, routes through `docker create`, and deletes it only after daemon acknowledgment, preventing a `$HOME` workspace mount from rewriting later launches. Crash leftovers are swept after one hour. The vendored Moby source's raw SHA-256 is `536529b665dd0972c37bfb569f5d4ac8a53592e7b00752bc39ff063ca9864c74`. Docker's documentation explicitly discourages `seccomp=unconfined`, including for recent syscall-filter compatibility workarounds.
