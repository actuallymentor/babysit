...
- **Model**: `--model provider/model`; credentials are scoped to the provider prefix. Babysit resolves explicit CLI model → configured route model → provider-compatible frontier fallback. Verified OpenCode 1.18.19 and a real OpenRouter request on 2026-08-22: direct OpenAI uses `openai/gpt-5.6-sol`, while OpenRouter must use `openrouter/openai/gpt-5.6-sol`.
