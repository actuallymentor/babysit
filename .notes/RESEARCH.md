...
- Chrome's Linux sandbox works as the non-root image user when Docker permits its namespace calls. The tested runtime materializes a current Moby-default-derived allowlist with only unconditional `clone` and `unshare` added, plus a 1 GiB `/dev/shm`; it retains seccomp, grants no `SYS_ADMIN`, and does not disable Chrome's own sandbox. Docker's documentation explicitly discourages `seccomp=unconfined`, including for recent syscall-filter compatibility workarounds.
