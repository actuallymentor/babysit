...
- Startup verifies only the active agent, but credential discovery stages all four agents serially and passes every staged credential into the active probe. Any present Claude credential also runs an unbounded synchronous host `claude --version` preflight on every launch; macOS adds synchronous Keychain reads. GitHub token capture is later but also synchronous, and its normal `gh auth token` fallback has no timeout.
...
- Official fast status surfaces verified 2026-08-21: `claude auth status`, `codex login status`, and `opencode auth list` (stored providers only). Gemini documents no equivalent noninteractive status command, so its cheap path must validate selected auth type and credential presence without claiming remote inference health. If deep probes remain, run minimal profiles: Claude `--bare` with no tools/MCP, Codex `--ignore-user-config`, Gemini without extensions/tools/MCP, and OpenCode `--pure` with update/model-fetch/plugin startup disabled.
