...
- `babysit list` command to list and number active sessions with name-or-id, coding-agent `running`/`idle` status, tmux attachment status, agent, and the deepest two working-directory levels. `babysit list --all` also shows the separate session ID and full tmux session name.
