...
- `babysit list` command to list active sessions with their optional names, session ids, and the coding agent running in them
- `babysit open <session_id|name>` command to open a tmux session attached to the given session id or exact human-readable name. With no selector it attaches to the only active session for the current directory; when several match, it prints numbered choices that can be selected with `babysit open N`. Note that `babysit resume` is used to resume exited sessions and uses the session id as the coding agent knows it, but `babysit open` is used to connect to active tmux sessions.
